...
TODO TALK WITH DYLAN
- resolve missing data in TASK, outcomeType, timestamp... 
- are these the result of 'exploded' utterances that were dual coded? <-- need to carry the other attributes across both obs 
- max of 2 detail-codes applied, correct?
- where are the flag codes?

```{r profile, results="asis", warning=FALSE, message=FALSE}
...
```{r SHAPE, warning=FALSE, error=FALSE}
...
*There are `r n_rows` coded utterances, representing `r n_unique` unique statements made by `r n_participants` in the study. `r coded_single` utterances were single-coded, while `r coded_double` utterances received two detail codes. No more than `r max_codes` were applied to any single utterance.*
...
#STACKED BAR BY TASK
by_task <- ggplot(df_summary, aes(x = TASK, y=c, fill= DATASET)) + 
  geom_col() + 
...
by_dataset <- ggplot(df_summary, aes(x = DATASET, y=c, fill= TASK)) + 
  geom_col() + 
  scale_fill_brewer(type="qual", palette = 1) +
  geom_text(aes(label=c), size = 3, hjust = 0.5, vjust = 1.5, position = "stack") 

(p_raw_utterances <- by_task + by_dataset) 
...
```{r UTTERANCE-DATASET, warning=FALSE, message=FALSE}
...
```{r UTTERANCE-TASK-DATASET, warning=FALSE, message=FALSE}
...
#PREP DATASET FOR VIS
df <- df_insights %>% mutate(
  top_code = fct_rev(top_code), #reorder
  pid = factor(pid, levels = c(
 "bjs827ee1u", "3r2sh20ei", "4728sjuiz", "7ACC0B75","92ghd48xe","iurmer28", "s294hoei", #HAPPINESS-FIRST
  ...
  data_order = recode(data_order, "ixn _ happiness"="space-first",
                                  ...
                                  "static _ happiness"="happiness-first",
                                  "static _ space"="space-first")) %>%  
  select(pid,uid,TASK,DATASET,top_code, data_order) %>% 
  #remove handful of rows missing data TODO RESOLVE W/ DYLAN
  na.omit() %>% arrange(data_order)
...
##[stacked bar] UTTERANCES by TYPE, TASK W/ COUNTS
ggplot (df, aes(DATASET,c, fill= top_code)) + 
  geom_col() + 
  facet_grid(.~df$TASK)+
  geom_text(aes(label = c), size = 3, hjust = 0.5, vjust = 1.5, position = "stack") +
...
#TOP CODES BY FACTORS + PARTICIPANTS
(parts <- gf_bar( pid ~ ., fill = ~top_code, data = df) %>% 
