...
#PAPER FIGURE
d <- df_joined %>% filter(code_topic =="PROCESS")
(p <- gf_bar(  ~ rep_simple, fill = ~(code_detail), data = d) %>% 
  gf_facet_grid( DATASET ~ TASK) + 
  ...
  theme_minimal() + labs(
    title = "Utterance-Representations DETAIL by FACTORS"
))
...
#PAPER FIGURE
d <- df_joined %>% filter(code_topic == "PROCESS") %>% filter(TASK=="ixn")

#PAPER FIGURE
#DOTPLOT—BW
( p <- ggplot(d, aes(x=relative_time, y = PNUM, color = ixn)) + 
  geom_point(alpha=0.5, size=3) +
  facet_grid(d$DATASET) +
  # scale_color_brewer(type="qual", palette = 3) +
...
#PAPER FIGURE
d <- df_joined %>% filter(code_topic =="DATASET")
(p <- gf_bar(  ~ rep_simple, fill = ~(code_detail), data = d) %>% 
  gf_facet_grid( DATASET ~ TASK) + 
  scale_fill_brewer(type="seq", palette = "Greens") +
  coord_flip() + 
...
#PAPER FIGURE
d <- df_joined %>% filter(code_topic == "DATASET") %>% filter(TASK=="ixn")
...
#PAPER FIGURE
d <- df_joined %>% filter(code_topic =="VARIABLE")
(p <- gf_bar(  ~ rep_simple, fill = ~(code_detail), data = d) %>% 
  gf_facet_grid( DATASET ~ TASK) + 
  scale_fill_manual(values=c("#5AB4AC","#AED8D2","#03655E"))+
  coord_flip() + 
...
#PAPER FIGURE
d <- df_joined %>% filter(code_topic == "VARIABLE") %>% filter(TASK=="ixn")
...
#### RELATIONSHIP-REPS-LOW

```{r}

#PAPER FIGURE
d <- df_joined %>% filter(code_topic =="RELATIONSHIP")
(p <- gf_bar(  ~ rep_simple, fill = ~(code_detail), data = d) %>% 
  gf_facet_grid( DATASET ~ TASK) + 
 scale_fill_manual(values=c(
              "#C5D3DE",
...
ggsave(p, file="figures/UTTERANCE-REP_detail_RELATIONSHIP_factors.png", width=6, height=4)


```

#### RELATIONSHIP-REPS-IXN
```{r}

#PAPER FIGURE
d <- df_joined %>% filter(code_topic == "RELATIONSHIP") %>% filter(TASK=="ixn")
